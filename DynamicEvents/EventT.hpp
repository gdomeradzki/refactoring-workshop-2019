...
public:
	EventT(T const &payload = T()) : m_payload(std::make_unique<T>(payload))
	{
	}

	EventT(T &&payload) : m_payload(std::make_unique<T>(std::forward<T>(payload)))
	{
	}
	...
	{
		return std::make_unique<EventT<T>>(*m_payload);
	}
