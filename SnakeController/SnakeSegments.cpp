...
namespace Snake {
    ...
    Segments::Segments(Direction direction,IPort& m_displayPort,IPort& m_scorePort)
            : m_headDirection(direction), m_displayPort(m_displayPort),m_scorePort(m_scorePort) {}
    ...
    bool Segments::isCollision(Position position) const {
        return m_segments.end() != std::find_if(m_segments.cbegin(), m_segments.cend(),
                                                [position](auto const &segment) {
                                                    return segment.x == position.x and segment.y == position.y;
                                                });
    }
    ...
    void Segments::removeTailSegmentIfNotScored(Position position, World& m_world)
    {
        if (position == m_world.getFoodPosition()) {
            ...
            m_world.m_foodPort.send(std::make_unique<EventT<FoodReq>>());
        } else {
            removeTailSegment();
        }
    }
