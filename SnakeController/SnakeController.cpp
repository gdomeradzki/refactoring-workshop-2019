      ...
      m_paused(false)
{
    ...
    m_segments = std::make_unique<Segments>(readDirection(istr), displayPort, foodPort, scorePort, *(m_world));
...
{
    bool isColl = m_segments->isCollision(payload<FoodInd>(*e).position);
    m_world->updateFoodPosition(payload<FoodInd>(*e).position, isColl, true);
}

void Controller::handleFoodResp(std::unique_ptr<Event> e)
{
    bool isColl = m_segments->isCollision(payload<FoodInd>(*e).position);
    m_world->updateFoodPosition(payload<FoodResp>(*e).position, isColl, false);
}
