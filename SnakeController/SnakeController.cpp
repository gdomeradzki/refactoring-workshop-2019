      ...
      m_paused(false)
{
    ...
    if (w == 'W' and f == 'F' and s == 'S') {
        myBoard.m_mapDimension = std::make_pair(width, height);
        myBoard.m_foodPosition = std::make_pair(foodX, foodY);
...
bool Board::isPositionOutsideMap(int x, int y) const
{
...
{
    myBoard.m_foodPosition = std::make_pair(x, y);
...
void Controller::sendClearOldFood()
{
    ...
    clearOldFood.x = myBoard.m_foodPosition.first;
    clearOldFood.y = myBoard.m_foodPosition.second;
...
{
    if (std::make_pair(newHead.x, newHead.y) == myBoard.m_foodPosition) {
...
{
    myBoard.updateFoodPosition(x,y,clearPolicy,this);
}


void Board::updateFoodPosition(int x, int y, std::function<void()> clearPolicy, Controller *myController)
{
    if (myController->isSegmentAtPosition(x, y) || isPositionOutsideMap(x,y) ) {
        myController->m_foodPort.send(std::make_unique<EventT<FoodReq>>());
        return;
    }

    myController->clearPolicy();
    myController->sendPlaceNewFood(x, y);
}
...
void Controller::handleFoodInd(std::unique_ptr<Event> e)
{
    ...
    Controller::updateFoodPosition(receivedFood.x, receivedFood.y, std::bind(&Controller::sendClearOldFood, this));
}
...
void Controller::handleFoodResp(std::unique_ptr<Event> e)
{
    ...
    Controller::updateFoodPosition(requestedFood.x, requestedFood.y, []{});
}
