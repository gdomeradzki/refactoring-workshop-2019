...
std::unique_ptr<World> readWorld(std::istream& istr, std::unique_ptr<Segments>& s, IPort& d, IPort& f)
{
    ...
    return std::make_unique<World>(worldDimension, foodPosition, s, d, f);
}
      ...
      m_paused(false)
{
    ...
    m_world = readWorld(istr, m_segments, displayPort, foodPort);
    m_segments = std::make_unique<Segments>(m_world, displayPort, foodPort, scorePort, readDirection(istr));
...
{
   m_world->updateFoodPosition(payload<FoodInd>(*e).position, std::bind(m_world->sendClearOldFood, this));
}
