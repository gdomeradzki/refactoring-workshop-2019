...
{
   m_world->updateFoodPosition(payload<FoodInd>(*e).position, std::bind(&World::sendClearOldFood, m_world.get()),m_segments->isCollision(payload<FoodInd>(*e).position));
}
...
void Controller::handleFoodResp(std::unique_ptr<Event> e)
{
    ...
    m_world->updateFoodPosition(payload<FoodResp>(*e).position, noCleanPolicy,m_segments->isCollision(payload<FoodResp>(*e).position));
}
