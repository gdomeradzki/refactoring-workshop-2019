...
void Controller::receive(std::unique_ptr<Event> e)
{
    try {
        ...
        bool lost = checkIfSelfCollision(newHead);

        if (not lost) {
            if (checkIfAteFood(newHead)) {
                m_scorePort.send(std::make_unique<EventT<ScoreInd>>());
                m_foodPort.send(std::make_unique<EventT<FoodReq>>());
            } else if (checkIfWallCollision(newHead)) {
        ...
        try {
            ...
            placeNewFood(std::move(e));
        }
    }
}

    Controller::Segment Controller::calculateNewHead() {
    ...
    bool Controller::checkIfSelfCollision(const Controller::Segment& newHead) {
    ...
    bool Controller::checkIfWallCollision(const Controller::Segment& newHead) {
        return newHead.x < 0
            or newHead.y < 0
    ...
    bool Controller::checkIfAteFood(const Controller::Segment& newHead) {
        return std::make_pair(newHead.x, newHead.y) == m_foodPosition;
    }

    void Controller::freeLastCell() {
        for (auto &segment : m_segments) {
            if (not --segment.ttl) {
                DisplayInd l_evt;
                l_evt.x = segment.x;
    ...
    void Controller::placeNewHead(const Controller::Segment& newHead) {
    ...
    void Controller::placeNewFood(std::unique_ptr<Event> e) {
        try {
