...
Controller::Controller(IPort& displayPort, IPort& foodPort, IPort& scorePort, std::string const& initialConfiguration)
    : ports(Ports{displayPort, foodPort, scorePort}),
      m_paused(false)
{
    std::istringstream istr(initialConfiguration);

    m_world = readWorld(istr);
    m_segments = std::make_unique<Segments>(readDirection(istr), ports);
...
void Controller::handleFoodInd(std::unique_ptr<Event> e)
{
    ...
    m_world->updateFoodPosition(payload<FoodInd>(*e).position, *m_segments, clear, ports.m_displayPort, ports.m_foodPort);
}
...
void Controller::handleFoodResp(std::unique_ptr<Event> e)
{
    ...
    m_world->updateFoodPosition(payload<FoodResp>(*e).position, *m_segments, clear, ports.m_displayPort, ports.m_foodPort);
}
