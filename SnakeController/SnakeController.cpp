      ...
      m_paused(false)
{
    ...
    if (w == 'W' and f == 'F' and s == 'S') {
        ...
        while (length--) {
            Segment seg;
            istr >> seg.x >> seg.y;
            snake.getCurrentDirection().push_back(seg);
        }
    } else {
        throw ConfigurationError();
    }
...
{
    return snake.getSegments().end() !=  std::find_if(m_segments.cbegin(), m_segments.cend(),
...
{
    snake.getFoodPosition()= std::make_pair(x, y);
...
void Controller::sendClearOldFood()
{
    ...
    clearOldFood.x = this->getFoodPosition().first;
    clearOldFood.y = this->getFoodPosition().second;
