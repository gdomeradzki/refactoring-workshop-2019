      ...
      m_paused(false)
{
    ...
    if (w == 'W' and f == 'F' and s == 'S') {
        word.m_mapDimension = std::make_pair(width, height);
        word.m_foodPosition = std::make_pair(foodX, foodY);
...
{
    return x < 0 or y < 0 or x >= word.m_mapDimension.first or y >= word.m_mapDimension.second;
}

void Controller::sendPlaceNewFood(int x, int y)
{
    word.m_foodPosition = std::make_pair(x, y);
...
void Controller::sendClearOldFood()
{
    ...
    clearOldFood.x = word.m_foodPosition.first;
    clearOldFood.y = word.m_foodPosition.second;
...
{
    if (std::make_pair(newHead.x, newHead.y) == word.m_foodPosition) {
