...
std::unique_ptr<World> readWorld(std::istream& istr, IPort& m_displayPort,IPort& m_foodPort )
{
    ...
    return std::make_unique<World>(worldDimension, foodPosition,m_displayPort,m_foodPort);
}
...
{
    //m_world->updateFoodPosition(payload<FoodInd>(*e).position, std::bind(&Controller::sendClearOldFood, this));
    //m_world->updateFoodPosition(payload<FoodInd>(*e).position, std::bind(&World::sendClearOldFood, this));
    m_world->updateFoodPosition(payload<FoodInd>(*e).position,*m_segments);
}
...
void Controller::handleFoodResp(std::unique_ptr<Event> e)
{
    ...
    m_world->updateFoodPosition(payload<FoodResp>(*e).position,*m_segments,noCleanPolicy);
