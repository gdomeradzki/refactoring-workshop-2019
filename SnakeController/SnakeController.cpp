...
auto Controller::handleRequestedFood(FoodResp requestedFood)
{
...
auto Controller::handleReceivedFood(FoodInd receivedFood)
{
    bool requestedFoodCollidedWithSnake = false;
    for (auto const& segment : m_segments) {
...
auto Controller::SendDisplayInd(Segment& newHead, bool& lost)
{
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                handleReceivedFood(receivedFood);

            } catch (std::bad_cast&) {
                try {
                    auto requestedFood = *dynamic_cast<EventT<FoodResp> const&>(*e);
                    handleRequestedFood(requestedFood);
                } catch (std::bad_cast&) {
